...
class DocumentoDataCollector {
    ...
    public function addSerieFiscal($data) { if ($data && !isset($this->seriesFiscal)) $this->seriesFiscal = $data; }
    public function addCliente($data) { if ($data && !isset($this->cliente)) $this->cliente = $data; }
    public function addVendedor($data) { if ($data && !isset($this->vendedor)) $this->vendedor = $data; }

    public function addVehiculo($data) { if ($data && !isset($this->vehiculo)) $this->vehiculo = $data; }
    public function addPaciente($data) { if ($data && !isset($this->paciente)) $this->paciente = $data; }
    ...
    public function addBanco($data, $sucursal) { if ($data && !isset($this->bancos[$data->codigo])) $this->bancos[$data->codigo] = $data; $this->sucursal = $sucursal; }
    
    public function addCentroCosto($data) { if ($data && !isset($this->sucursal)) $this->sucursal = $data; }
