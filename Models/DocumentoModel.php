...
class DocumentoModel extends Mysql {
    ...
    // =================================================================================
    public function prepararDatosTransaccionalesFactura($factura, $moneda_base) {
        $documentos = [];
        $movimientos = [];
        $movimientos_recibo = [];
        $documentosIgtf = [];
        ...
        $documentos[] = $this->_prepararArrayDocumento($factura, 'PED', 'D', $moneda_base, ['numeroPedUltimo' => $numeroPedUltimo, 'horaPed' => $horaPed['tiempo']]);
        
        //PROCESO DE REGISTRA LAS D BIEN SEA NEN O FAV
        if($factura->tipo_documento === 'NEN'){
            $documentos[] = $this->_prepararArrayDocumento($factura, 'NEN', 'D', $moneda_base, ['numeroPedUltimo' => $numeroPedUltimo, 'horaFav' => $horaFav['tiempo']]);
    ...
    private function _prepararArrayDocumento($factura, $tipo_documento_procesar, $tipo_doc, $base_moneda, $overrides = []) {
        ...
        if($factura->empresa === 'MPC' || $factura->empresa === 'MPC_TEST'){
    ...
    protected function _prepararArrayRecibo($factura, $tipo_documento, $overrides = [])
    {
    ...
    protected function _prepararArrayMovimientoRecibo($factura, $movimiento, $tipo_documento, $overrides = [])
    {   
        $activo = ($tipo_documento == 'NEN') ? '0' : '1';
    ...
    private function _bulkInsertOrUpdateSimpleSerieFiscal(string $tabla, array $columnas, $data) {
        ...
        $sql_final = " ON DUPLICATE KEY UPDATE SFI_MODELO = VALUES(SFI_MODELO)"; // Actualiza la segunda columna (descripción)
    ...
    // =================================================================================
    private function _bulkInsertUndagruLogic(DocumentoDataCollector $data) {
    ...
    private function _bulkInsertOrUpdateClientes($data) {
        ...
        $sql = "SELECT count(*) as total FROM adn_clientes WHERE CLT_CODIGO = '{$data->codigo}'";
        $result = $this->select($sql);
        ...
        $values = [
            $this->conexion->quote($data->codigo), 
            $this->conexion->quote($data->nombre ?? ''), 
            $this->conexion->quote($data->rif ?? ''),
            $this->conexion->quote($data->direccion ?? ''), // El JSON no tiene el '1'
            $this->conexion->quote($data->telefono ?? ''),  // El JSON no tiene el '1'
            $this->conexion->quote($data->celular ?? ''), 
            $this->conexion->quote($data->email ?? ''),
            $this->conexion->quote($data->tipo_persona ?? 'N'), // Nuevo campo
            $this->conexion->quote($data->activo ?? '1'),
            $this->conexion->quote($data->clasificacion_contribuyente ?? '000001'), // Nuevo campo
            $this->conexion->quote($data->condicion ?? 'CONTADO')
        ];
    ...
    private function _bulkInsertOrUpdateVendedores($data) {
        ...
        $values = [
            $this->conexion->quote($data->codigo), 
            $this->conexion->quote($data->nombre ?? ''),
            $this->conexion->quote($data->apellido ?? ''),
            $this->conexion->quote($data->activo ?? '1'),
            $this->conexion->quote($data->email ?? ''),
            $this->conexion->quote($data->telefono ?? '')
        ];
        $valueStrings[] = "(" . implode(',', $values) . ")";
